...
/**
 * REST controller for managing {@link com.alteca.gestion.domain.Participant}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ParticipantResource {
    ...
    /**
     * {@code GET  /participants/ByTache/:idTache} : get all the the participants from a tache.
     *
     * @param idTache the id of the tache to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the participant, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/participants/ByTache/{idTache}")
    public List<Participant> getParticipantByTache(@PathVariable Long idTache) {
        log.debug("REST request to get Participant : {}", idTache);
        return participantRepository.findByTache(idTache);
    }
